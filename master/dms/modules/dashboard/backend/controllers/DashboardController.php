	...
	class DashboardController extends BackendController {
		...
		public function index() {
			...
			$recents = Logs::model()->findAll(array(
				...
				'condition' => 'is_deleted = 0 AND created_by = :id AND act = "open" AND logs_id IN (SELECT MAX(logs_id) FROM tbl_logs GROUP BY file_target_id) AND file_target_id IN (SELECT folder_id FROM tbl_folder WHERE is_deleted = 0) ORDER BY created_on DESC',
				'params'	=> array(':id' => Snl::app()->user()->user_id)
			));
